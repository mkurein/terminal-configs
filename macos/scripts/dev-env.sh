...
case $PROJECT_TYPE in
    ...
    ruby)
        echo "💎 Ruby проект обнаружен"
        echo ""
        
        if [[ ! -d "vendor/bundle" ]]; then
            read -p "Установить зависимости? (bundle install) [y/N]: " install
            if [[ "$install" == "y" ]]; then
                bundle install
            fi
        fi
        ...
        echo ""
        echo "Доступные команды:"
        echo "  bundle exec rails server - Rails сервер"
        echo "  bundle exec rake test     - тесты"
        ;;
        
    swift)
        echo "🍎 Swift проект обнаружен"
        echo ""
        echo "Доступные команды:"
        echo "  swift build    - сборка"
        echo "  swift run      - запуск"
        echo "  swift test     - тесты"
        
        if command -v xcodebuild &> /dev/null; then
            echo "  xcodebuild     - сборка через Xcode"
        fi
        ;;
