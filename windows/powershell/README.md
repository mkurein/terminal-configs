...
### Приглашение (ветка git)

По умолчанию PowerShell пишет только `PS C:\path>` и **не** показывает ветку. В профиле есть git-prompt: в репозитории будет `PS C:\Project\homelab-book [main]>`. Cursor запоминает prompt при открытии вкладки — уже открытое окно подхватит ветку только после `. $PROFILE` (или нового терминала). Подробности — в начале корневого [`README.md`](../../README.md) (раздел «Терминал: ветка и `. $PROFILE`»).
