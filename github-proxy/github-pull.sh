#!/usr/bin/env bash
# command not found: github-pull — не PATH.  source ~/.zshrc
# или: source "$HOME/Project/terminal-configs/github-proxy/env.sh"
# напрямую: "$HOME/Project/terminal-configs/github-proxy/github-pull.sh"
# Windows: . $PROFILE
set -euo pipefail
# shellcheck source=_lib.sh
. "$(cd "$(dirname "$0")" && pwd)/_lib.sh"

github_proxy_require_git
github_proxy_clear

echo "Proxy variables cleared for this terminal session."
echo "Running: git -c http.proxy= -c https.proxy= pull"

git -c http.proxy= -c https.proxy= pull "$@"
