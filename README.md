...
**Несколько строк нельзя вставлять в приглашение shell** (после `❯` / `PS>`). Сначала запусти `github-commit` **без аргументов**, потом пиши текст.

Пустая строка **не обязательна**. Тема и сразу конец ввода — достаточно.
...
git add path/to/file
github-commit
# одна строка темы, Ctrl-D
# если нужен абзац: тема, пустая строка, тело, Ctrl-D
```
...
**Windows:** `github-commit` без аргументов → строка темы → строка только `.` (пустая не нужна). Или `github-commit -e`.
